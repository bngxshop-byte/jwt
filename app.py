...
def get_token():
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                "status": "error",
                "message": f"Invalid response from Garena: {str(e)}",
        ...
        if "error" in garena_data:
            return jsonify({
                "status": "error",
                "message": f"Garena error: {garena_data['error']}",
        ...
        if "access_token" not in garena_data or "open_id" not in garena_data:
            return jsonify({
                "status": "error",
                "message": "Missing access_token or open_id",
                "garena_response": garena_data
            })
        ...
        if token:
            ...
            return jsonify({
                "status": "failure",
                "message": "Failed to generate final token",
                "garena_info": {
    ...
    return jsonify({
        ...
        "endpoints": {
            "/get": {
                ...
                "description": "Generate final token"
            },
            "/test": {
                "method": "GET",
                "params": ["uid", "password"],
                "example": "/test?uid=123456&password=abc123",
                "description": "Test Garena login only"
            },
            "/health": {
                "method": "GET",
